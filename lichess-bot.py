...
def play_game(li, game_id, control_queue, engine_factory, user_profile, config, challenge_queue, correspondence_queue, logging_queue, logging_configurer, logging_level):
    ...
    conversation.send_reply(SendLine('player'), f'Made by @Servan5635')
    conversation.send_reply(SendLine('player'), f'Good Luck {opponent}!')
    conversation.send_reply(SendLine('player'), f'Type !help for list of commands I can Reply to.Thanks for playing with me')
    conversation.send_reply(SendLine('spectator'), f'Made by @Servan5635')
