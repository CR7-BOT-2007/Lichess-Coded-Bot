...
class Conversation:
    ...
    def command(self, line, game, cmd):
        if cmd == "commands" or cmd == "help":
            self.send_reply(line, "Supported commands: !wait(This will only work before your first move), !name, !howto, !queue,!commands,!eval")
        elif cmd == "wait" and game.is_abortable():
            game.ping(60, 120)
            self.send_reply(line, "Waiting 60 seconds...")
        elif cmd == "name":
            name = game.me.name
            self.send_reply(line, "{} running {} (lichess-bot v{} Programmed by @EshanHasaranga200703 and @Javan727) ".format(name, self.engine.name(), self.version))
        elif cmd == "howto":
